...
struct SettingsView: View {
	...
	@State private var userInfo: UserModel = UserModel()
			...
			VStack {
				ZStack {
					...
					Image("profileSec2")
						...
						.clipped()
						.accessibilityIdentifier("profileSec2")
				}
				.padding()
				
				Form {
					Section(header: Text("Personal Information").font(.headline).padding(.top, 10).foregroundColor(.secondary)) {
					...
					Section {
						Button(action: toggleEditMode) {
							Text(isEditing ? "Save" : "Edit")
								...
								.cornerRadius(10)
								.padding(.vertical, 5)
								.accessibility(identifier: "editSaveButton")
						}
			...
			HStack {
				...
				if isEditing {
					TextField("Enter your \(label.lowercased())", text: $text)
						...
						.accessibilityIdentifier("\(label.lowercased())TextField")
						.frame(width: 200)
				} else {
					Text(text)
						.accessibilityIdentifier("\(label.lowercased())Text")
				}
			}
			.padding(.vertical, 5)
		}
			...
			HStack {
				...
				if isEditing {
					TextField("Enter your \(label.lowercased())", value: $value, formatter: NumberFormatter())
						.textFieldStyle(RoundedBorderTextFieldStyle())
						.accessibility(identifier: "\(label.lowercased())IntField")
						.frame(width: 200)
				} else {
					Text("\(value)")
						.accessibilityIdentifier("\(label.lowercased())IntText")
				}
			}
			.padding(.vertical, 5)
		}
		...
		var body: some View {
			HStack {
				...
				if isEditing {
					TextField("Enter your \(label.lowercased())", value: $value, formatter: NumberFormatter())
						.textFieldStyle(RoundedBorderTextFieldStyle())
						.accessibility(identifier: "\(label.lowercased())DoubleField")
						.frame(width: 200)
				} else {
					Text("\(value, specifier: "%.1f")")
						.accessibilityIdentifier("\(label.lowercased())DoubleText")
				}
			}
			.padding(.vertical, 5)
		}
